...
//function player_health_item(player, health) {
//        health.kill();
...
function update() {
    ...
    group_health_item.forEachAlive(function (heath_item) {
        if (game.physics.arcade.overlap(heath_item, player)) {
            var health;
            health = player_health_group.getFirstExists();
            ////player.animations.play('hurt');
...
function render() {
    ...
    game.debug.text('Injury Timer: ' + injuryTimer, 32, 128);
